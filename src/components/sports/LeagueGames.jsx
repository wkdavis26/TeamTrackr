...
async function fetchPWHLGames() {
  try {
    ...
    console.log('[PWHL] Total games from API:', games.length);
    ...
    console.log('[PWHL] Upcoming games after filter:', result.length, result.map(g => g.date.toISOString()));
    return result;
  } catch (e) {
    console.error('[PWHL] Error:', e);
    return [];
  }
}

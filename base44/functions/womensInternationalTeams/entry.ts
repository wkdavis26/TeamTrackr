...
Deno.serve(async (req) => {
  try {
    ...
    // FIFA Women's World Cup (6) as base team list
    const res = await fetch(`${BASE_URL}/teams?league=6&season=${season}`, {
    ...
    const teams = (data.response || []).map(t => ({
      id: `womens-international-football-${nameToSlug(t.team?.name || '')}`,
      name: t.team?.name,

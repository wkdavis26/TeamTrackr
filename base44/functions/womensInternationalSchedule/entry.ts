...
// Women's International: FIFA Women's World Cup (6), UEFA Women's Euro (36), Women's Friendlies (668)
const WOMENS_LEAGUES = [6, 36, 668];
...
Deno.serve(async (req) => {
  try {
    ...
    const allData = await Promise.all(
      WOMENS_LEAGUES.map(id => apiFetch(`/fixtures?league=${id}&season=${season}`).catch(() => null))
    );
    ...
    const games = allData
      ...
      .map(g => ({
        ...
        homeTeam: {
          id: `womens-international-football-${nameToSlug(g.teams.home.name)}`,
          name: g.teams.home.name,
          logo: g.teams.home.logo || null,
        },
        awayTeam: {
          id: `womens-international-football-${nameToSlug(g.teams.away.name)}`,
